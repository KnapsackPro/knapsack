describe Knapsack::AllocatorBuilder do
  ...
  let(:adapter_spec_pattern) { adapter_class::TEST_DIR_PATTERN }

  let(:env_ci_node_total) { double }
  let(:env_ci_node_index) { double }
  let(:env_report_path) { nil }
  let(:env_spec_pattern) { nil }
  ...
  describe '#allocator' do
    ...
    before do
      ...
      expect(Knapsack::Config::Env).to receive(:spec_pattern).and_return(env_spec_pattern)
    ...
    shared_examples 'allocator builder' do
      context 'when ENVs are nil' do
        ...
        let(:allocator_args) do
          {
            report: report,
            spec_pattern: adapter_spec_pattern,
      ...
      context 'when ENV spec_pattern has value' do
        let(:env_spec_pattern) { 'custom_spec/**/*_spec.rb' }
        let(:report_config) { { report_path: adapter_report_path } }
        let(:allocator_args) do
          {
            report: report,
